...
const urlByNameAndDateNew = (props: UrlAnalyticsByNameAndDatePropsNew) => {
  ...
  const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}/${analyticsType}/${process.env.NEXT_PUBLIC_API_BASE_URL_RESULTS_GROUPED}${testFormatFix(name)}&startDate=${startDate}&endDate=${endDate}&pageSize=200&sort=date,asc`
