...
const GeneratePdf: React.FC<PdfGeneratorProps> = ({
  ...
  const formatValue = (value: number | string | null | undefined): string | number => {
    ...
    try {
      ...
      if (jsonData && Array.isArray(jsonData)) {
        if (jsonData.length > 0) {
          ...
          const data = jsonData.map((item: AnalyticItem) =>
            headers.map((header) => formatValue(item[header as keyof AnalyticItem]))
          )
      ...
      if (error instanceof Error) {
        const errorMsg =
          'status' in error ? getStatusMessage((error as { status: number }).status) : error.message
