...
export function useFetchSWR<T>({
  ...
  const fetchKey =
    immediate && (!authenticated || (authenticated && token && !isTokenLoading))
      ? [url, method, body ? JSON.stringify(body) : undefined]
      : null
  ...
  const { data, error, isLoading, mutate, isValidating } = useSWR(fetchKey, fetcher, {
